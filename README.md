...
| `HOUTINI_LM_THINKING` | `auto` | Thinking control: `auto` detects thinking support and suppresses it for recognised local models, `off` forces the no-think path for every call, and `on` explicitly sends `enable_thinking: true`. Use `off` when an orchestrator (e.g. Claude) does the reasoning and the local model only executes — and for vLLM served under an alias (e.g. `coder-next`), where HF-metadata detection cannot identify the real model. |
